<div style="margin: 10px;" id="mcq">


  <?php

  if (count($questions) > 0) {
    // output data of each row
    foreach ($questions as $question) { ?>
      <div class="divide-10"></div>
      <h5><strong><a href="<?php echo site_url(ADMIN_DIR . "home/topics/$question->topic"); ?>"><?php echo $question->topic; ?> </a> / <?php echo $question->category; ?></strong></h5>
      <hr />
      <p><?php echo $question->question; ?></p>
      <div class="row" style="margin-top: 15px;">
        <div class="col-sm-6" onclick="check_answer('<?php echo $question->option_1; ?>')">
          <small class="list-group-item" style="margin:2px; border-left: 40px solid hsl(27deg 91% 55%);" id="<?php echo $question->option_1; ?>">
          <span style="margin-left: -45px; position: absolute; font-size: 25px; margin-top: -10px;  color:white;">1.</span>
          <?php echo $question->option_1; ?> </small>
        </div>
        <div class="col-sm-6" onclick="check_answer('<?php echo $question->option_2; ?>')">
          <small class="list-group-item" style="margin:2px; border-left: 40px solid hsl(27deg 91% 55%);" id="<?php echo $question->option_2; ?>">
          <span style="margin-left: -45px; position: absolute; font-size: 25px; margin-top: -10px;  color:white;">2.</span>
          <?php echo $question->option_2; ?> </small>
        </div>
        <div class="col-sm-6" onclick="check_answer('<?php echo $question->option_3; ?>')">
          <small class="list-group-item" style="margin:2px; border-left: 40px solid hsl(27deg 91% 55%);" id="<?php echo $question->option_3; ?>">
            <span style="margin-left: -45px; position: absolute; font-size: 25px; margin-top: -10px;  color:white;">3.</span>
            <?php echo $question->option_3; ?> </small>
        </div>
        <div class="col-sm-6" onclick="check_answer('<?php echo $question->option_4; ?>')">

          <small class="list-group-item" style="margin:2px; border-left: 40px solid hsl(27deg 91% 55%) " id="<?php echo $question->option_4; ?>">
            <span style="margin-left: -45px; position: absolute; font-size: 25px; margin-top: -10px; color:white;">4.</span>
            <?php echo $question->option_4; ?> </small>
        </div>
      </div>
      <br />
      <p id="answer_detail" style="display: none; margin-top: 10px; " >
        <strong style="margin-top: 10px; margin-bottom: 10px; "> 
            <span onclick="$('#correct_answer').show()">
            <i class="fa fa-info-circle" aria-hidden="true"  ></i> Correct Answer:</span> 
        <span id="correct_answer" style="display: none;"><?php echo $question->answer; ?></span>
        </strong>
        
        <br />
        <br />

        <?php echo $question->answer_detail; ?>
        <p id="next_button" style="text-align: center;"></p>
      </p>
  <?php }
  } else {
    echo "You Complete this Topic Would you like to try again?"
  }

  ?>
  <script>
    function check_answer(answer) {
      correct_answer = '<?php echo $question->answer; ?>';
      mcq_id = '<?php echo $question->id; ?>';
      user_answar = answer;
      if (correct_answer === user_answar) {
        document.getElementById(user_answar).style.backgroundColor = "#90ee90";
        document.getElementById("answer_detail").style.display = "block";
        var snd = new Audio("data:audio/mpeg;base64,");
        snd.play();
        $.ajax({
          type: "POST",
          url: '<?php echo site_url(ADMIN_DIR . "home/add_user_mcqs"); ?>',
          data: {
            mcq_id: mcq_id,
            correct: 1
          }
        }).done(function(data) {

          $('#correct_answer').show();
          $('#next_button').html('<button class="btn btn-warning" onclick="location.reload();">Next Quesiton</button>');
          //alert(data);
        });


      } else {
        document.getElementById(user_answar).style.backgroundColor = "#ffcccb";
        document.getElementById("answer_detail").style.display = "block";
        var snd = new Audio("data:audio/wav;base64,");
        snd.play();
        $.ajax({
          type: "POST",
          url: '<?php echo site_url(ADMIN_DIR . "home/add_user_mcqs"); ?>',
          data: {
            mcq_id: mcq_id,
            correct: 0
          }
        }).done(function(data) {
          //location.reload();
          //$('#mcq').html(data);
          //alert(data);
        });

      }
    }
  </script>
</div>